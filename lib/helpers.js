...
var walk = function(dir, options, done) {
  ...
  fs.readdir(dir, function(err, list) {
    ...
    if (!pending) return done(null, results, noCompileResults);
    list.forEach(function(file) {
      file = path.resolve(dir, file)

      var shouldIgnore = false
      if (options.ignore) {
        shouldIgnore = options.ignore.some(function(ignorePath) {
          return file.indexOf(ignorePath) !== -1
        })
      }

      if (shouldIgnore) {
        results.push(file)
        if (!--pending) done(null, results, noCompileResults)
      } else {
...
exports.ls = function(dir, options, callback) {
  ...
  walk(dir, options, function(err, results, noCompileResults){
    var files = []
    var noCompileFiles = []
    results.map(function(file){ files.push(path.relative(dir, file)) })
    noCompileResults.map(function(file){ noCompileFiles.push(path.relative(dir, file)) })
    callback(null, files, noCompileFiles)
  })
}
