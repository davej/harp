...
exports.process = function(req, rsp, next){
  ...
  if(!sourceFile && !isSrcMap){
    ...
    if (isSrcMap) {
      console.log('this is a source map')
      var cacheHit = srcMapCache[normalizedPath]
      if (cacheHit) {
        console.log('and it\'s in the cache')
        return render(200, normalizedPath, cacheHit)
      }
    }
    ...
    req.poly.render(sourceFile, function(error, body, srcMap){
      if(error){
        ...
        // If this file has an associated source map
        if (srcMap) {
          // Add the source map to a cache because otherwise we will be
          // double-compiling everytime.
          //
          // There is an edge case where we get an old source map if
          // we do something like:
          //   * request compiled file
          //   * change compiled file
          //   * request source map file
          //
          // In practice this isn't really an issue because browsers will always
          // request the compiled file before the source map file.
          //
          // TODO: Caching should apply to all compiled files (not just
          // the source maps) and should be linked to file changes.
          var srcMapPath = normalizedPath + ".map"
          srcMapCache[srcMapPath] = srcMap
        }

        if (isSrcMap && srcMap) {
          // This is a source map but was not in the cache (probably wont happen often).
