...
exports.process = function() {
  // TODO: Perhaps we should dump cache when it reaches a `cacheByteSize` threshold
  var cache = {};
  var cacheByteSize = 0;
  var watchers = {};
  ...
  return function(req, rsp, next){
    ...
    console.log(normalizedPath, normalizedPath in cache)

    if (normalizedPath in cache) {
      return render(200, normalizedPath, cache[normalizedPath])
    }

    var priorityList    = terraform.helpers.buildPriorityList(normalizedPath)
    var sourceFile      = terraform.helpers.findFirstFile(req.setup.publicPath, priorityList)
    var isSrcMap        = path.extname(normalizedPath).replace(/^\./, '') === 'map'

    /**
     * We GTFO if we don't have a source file.
     */

    if(!sourceFile && !isSrcMap){
      ...
      req.poly.render(sourceFile, function(error, body, srcMap){
        if(error){
          error.stack = helpers.stacktrace(error.stack, { lineno: error.lineno })
          ...
          if (isSrcMap && srcMap) {
            render(200, normalizedPath, srcMap)
            cacheAndWatch(sourceFile, body, srcMap)
          } else {
            render(200, sourceFile, body)
            cacheAndWatch(sourceFile, body, srcMap)
          }
    ...
    function render(statusCode, path, body) {
      var outputType = terraform.helpers.outputType(path)
    ...
    function deleteCache(outputPath) {
      if (outputPath in cache) {
        cache[srcMapPath] = null
        delete cache[outputPath]
      }
      var srcMapPath = outputPath + '.map'
      if (srcMapPath in cache) {
        cache[srcMapPath] = null
        delete cache[srcMapPath]
      }
    }

    function cacheAndWatch(localPath, body, srcMap) {
      // Cache
      var outputPath = '/' + terraform.helpers.outputPath(localPath)
      cache[outputPath] = body
      cacheByteSize += body.length
      if (srcMap) {
        var srcMapPath = outputPath + '.map'
        cache[srcMapPath] = srcMap
        cacheByteSize += srcMap.length
      }
      // Watch (on change: reload and delete cache entry)
      var sources;
      if (srcMap) {
        var sources = JSON.parse(srcMap).sources.map(function(src) {
          return path.dirname(localPath) + '/' + src
        })
      } else {
        sources = [localPath]
      }
      ...
      if (!(outputPath in watchers)) {
        chokidar.watch(sources, {
          cwd: req.projectPath
        }).on('change', function() {
          deleteCache(outputPath)
          req.app.reload(outputPath)
        })
        watchers[outputPath] = true
      }
    }
